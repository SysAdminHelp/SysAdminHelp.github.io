...
Below is an example of a query coming from one endpoint with no corresponding records for it at the TLD/Auth NS level - so it iterates immediately to a root server from a local DNS Server and then recurses down to the target host after reaching the other host's Auth NS
